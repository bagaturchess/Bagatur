...
public class SearchInfoUtils {
	...
	public static String buildMajorInfoCommand_multipv(int pvnum, ISearchInfo info, long startTime, int tptusage) {
		return buildMajorInfoCommand(info, startTime, tptusage, "info multipv " + pvnum);
	}
	
	
	public static String buildMajorInfoCommand(ISearchInfo info, long startTime, int tptusage) {
		return buildMajorInfoCommand(info, startTime, tptusage, "info");
	}
	
	private static String buildMajorInfoCommand(ISearchInfo info, long startTime, int tptusage, String prefix) {
	...
	public static String buildMinorInfoCommand(ISearchInfo info, long startTime, int tptusage) {
		long time = (System.currentTimeMillis() - startTime);
		long timeInSecs = (time / 1000);
		if (timeInSecs == 0) {
			timeInSecs = 1;
