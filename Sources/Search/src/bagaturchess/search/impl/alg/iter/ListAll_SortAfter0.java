...
public class ListAll_SortAfter0 implements ISearchMoveList {
	...
	*/
	...
	public ListAll_SortAfter0(SearchEnv _env, OrderingStatistics _orderingStatistics) { 
		env = _env;
		moves = new long[256];
	...
	public int next() {
		...
		if (cur < size) {
			if (cur == 0) {
				if (env.getSearchConfig().randomizeMoveLists()) Utils.randomize(moves, 0, size);
				if (env.getSearchConfig().sortMoveLists()) Sorting.bubbleSort(0, size, moves);
			}
