 ...
 *
 */
package bagaturchess.search.impl.alg.impl_progressive;
...
public class SearchMTD11 extends SearchImpl_MTD {
	...
	public SearchMTD11(Object[] args) {
		this(new SearchEnv((IBitBoard) args[0], getOrCreateSearchEnv(args)));
	}
	
	public SearchMTD11(SearchEnv _env) {
		super(_env);
	}
	...
	private IEvaluator_Progressive getEvaluatorProgressive() {
		return (IEvaluator_Progressive) env.getEval();
	}
	...
	public int pv_search(ISearchMediator mediator, IRootWindow rootWin, ISearchInfo info, int initial_maxdepth,
		...
		if (cur_move != 0) {
			do {
				...
				getEvaluatorProgressive().enterLevel(getDepthLevel(Math.max(initial_maxdepth, maxdepth), depth + 1));
				
				if (env.getBitboard().isInCheck(colourToMove)) {
					if (allowIllegalMoves()) {
						env.getBitboard().makeMoveBackward(cur_move);
						getEvaluatorProgressive().exitLevel(getDepthLevel(initial_maxdepth, depth + 1));
						continue;
					} else {
						throw new IllegalStateException();	
					}
	...
	private double getDepthLevel(int maxdepth, int depth) {
		double result =  depth / (double) normDepth(maxdepth);
		if (result > 1) {
			result = 1;
		}
		return result;
	...
	private int pv_qsearch(ISearchMediator mediator, ISearchInfo info, int initial_maxdepth, int depth, int alpha_org, int beta, int matgain, int initialStaticEval, boolean firstTime, int rootColour) {
		...
		do {
			...
			if (inCheck
				...
				env.getBitboard().makeMoveForward(cur_move);
				getEvaluatorProgressive().enterLevel(getDepthLevel(initial_maxdepth, depth + 1));
