	...
	public int calculateBestMove(ISearchMediator mediator, ISearchInfo info,
        ...
        while ((move = mp.next_move()) != 0) {
                ...
                && (!captureOrPromotion || moveCountPruning))
            {
                ...
                if (ply > 0 && stack[ply - 1].moveCount > 15)
                    r -= 1;
                ...
                int d = Math.max(newDepth - Math.max(r, 0), 1);

                value = -calculateBestMove(mediator, info, pvman, evaluator, cb, moveGen, ply + 1, d, -alpha - 1, -alpha, false, true);
            ...
            if (isPv && (moveCount == 1 || (value > alpha && (rootNode || value < beta))))
            {
                //stack[ply + 1].pv = pv;
                //stack[ply + 1].pv[0] = 0;

            	value = -calculateBestMove(mediator, info, pvman, evaluator, cb, moveGen, ply + 1, newDepth, -beta, -alpha, isPv, false);
		...
		if (bestMove != 0)
	    {
	        ...
	        //if (stack[ply - 1].moveCount == 1 && !pos.captured_piece())
	  ...
	  void update_quiet_stats(int ply, Stack[] stack, int bestMove, int[] quiets, int quietsCnt, int bonus) {
	    ...
	    mainHistory.array[us][env.getBitboard().getMoveOps().getFromFieldID(bestMove) * env.getBitboard().getMoveOps().getToFieldID(bestMove)]
	    		= mainHistory.array[us][env.getBitboard().getMoveOps().getFromFieldID(bestMove) * env.getBitboard().getMoveOps().getToFieldID(bestMove)] + bonus;
	    ...
	    {
	        mainHistory.array[us][env.getBitboard().getMoveOps().getFromFieldID(quiets[i]) * env.getBitboard().getMoveOps().getToFieldID(quiets[i])]
	        		= mainHistory.array[us][env.getBitboard().getMoveOps().getFromFieldID(quiets[i]) * env.getBitboard().getMoveOps().getToFieldID(quiets[i])] + -bonus;
