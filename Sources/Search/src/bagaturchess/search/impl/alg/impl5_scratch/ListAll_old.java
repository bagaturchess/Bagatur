...
public class ListAll_old implements ISearchMoveList {
	...
	private static final int ORD_VAL_MAX			=	100000000;
	...
	*/
	...
	public ListAll_old(SearchEnv _env, OrderingStatistics _orderingStatistics) { 
		env = _env;
		moves = new long[256];
	...
	public void reserved_add(int move) {
		...
		long move_ord = MoveInt.addOrderingValue(move, (int) ordval);
	 ...
	 private long genOrdVal(int move) {
		...
		if (MoveInt.isCapture(move) && MoveInt.getCapturedFigureType(move) == Constants.TYPE_KING) {
			ordval += ORD_VAL_MAX / 10;//env.getBitboard().getMaterialFactor().getMaterialGain(move);
			//ordval += 2 * BaseEvalWeights.getFigureMaterialSEE(Constants.TYPE_KING);
			//ordval += ORD_VAL_MAX;
		}
		
		if (ordval > ORD_VAL_MAX) {
			//throw new IllegalStateException("ordval=" + ordval);
			//ordval = ORD_VAL_MAX;
		}
		...
		//if (max_ordval < ordval) {
		//	max_ordval = ordval;
			//System.out.println("max_ordval=" + max_ordval + "	" + (int)max_ordval);
		//}

		return ordval;
	}
	
	//static long max_ordval = 0;
