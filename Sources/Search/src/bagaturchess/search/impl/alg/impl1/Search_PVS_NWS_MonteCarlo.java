...
public class Search_PVS_NWS_MonteCarlo extends SearchImpl {
	...
	public Search_PVS_NWS_MonteCarlo(Object[] args) {
		this(new SearchEnv((IBitBoard) args[0], getOrCreateSearchEnv(args)));
	}
	
	
	public Search_PVS_NWS_MonteCarlo(SearchEnv _env) {
		super(_env);
	}
	...
	private int search_MonteCarlo(ISearchMediator mediator, ISearchInfo info,
		...
		int WINDOW = 0;
		int MAX_ITERATIONS = Math.max(1, ply);
		...
		for (int j = 0; j < MAX_ITERATIONS; j++) {
			...
			for (int i=0; i<PLIES; i++) {
				int alpha_mc = (i % 2 == 0 ? alpha - WINDOW : -beta - WINDOW);
				int beta_mc = (i % 2 == 0 ? beta + WINDOW: -alpha + WINDOW);
				int score = (i % 2 == 0 ? 1 : -1)
