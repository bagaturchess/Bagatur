...
public class BagaturEvaluator_PiecesAttacks extends EvaluatorAdapter implements FeatureWeights {
	...
	BagaturEvaluator_PiecesAttacks(IBitBoard _bitboard, IEvalCache _evalCache, IEvalConfig _evalConfig) {
	...
	private void eval_mobility() {
	    ...
	    int count_attacks_minor_minor = 0;
	    int count_attacks_minor_major = 0;
	    int count_attacks_major_minor = 0;
	    int count_attacks_major_major = 0;
		...
		if (w_knights_count > 0) {
			...
			for (int i=0; i<w_knights_count; i++) {
			    ...
			    // Attacks
			    count_attacks_minor_minor += Utils.countBits(attacks & (evalInfo.bb_b_knights | evalInfo.bb_b_bishops));
			    count_attacks_minor_major += Utils.countBits(attacks & (evalInfo.bb_b_rooks | evalInfo.bb_b_queens));
		...
		if (b_knights_count > 0) {
			...
			for (int i=0; i<b_knights_count; i++) {
			    ...
			    // Attacks
			    count_attacks_minor_minor -= Utils.countBits(attacks & (evalInfo.bb_w_knights | evalInfo.bb_w_bishops));
			    count_attacks_minor_major -= Utils.countBits(attacks & (evalInfo.bb_w_rooks | evalInfo.bb_w_queens));
		...
		if (w_rooks_count > 0) {
			...
			for (int i=0; i<w_rooks_count; i++) {
			    ...
			    //Attacks
			    count_attacks_major_minor += Utils.countBits(attacks & (evalInfo.bb_b_knights | evalInfo.bb_b_bishops));
			    count_attacks_major_major += Utils.countBits(attacks & (evalInfo.bb_b_rooks | evalInfo.bb_b_queens));
		...
		if (b_rooks_count > 0) {
			...
			for (int i=0; i<b_rooks_count; i++) {
			    ...
			    //Attacks
			    count_attacks_major_minor -= Utils.countBits(attacks & (evalInfo.bb_w_knights | evalInfo.bb_w_bishops));
			    count_attacks_major_major -= Utils.countBits(attacks & (evalInfo.bb_w_rooks | evalInfo.bb_w_queens));
		...
		eval_o += (	count_attacks_minor_minor * ATTACKS_MINOR_MINOR
					+ count_attacks_minor_major * ATTACKS_MINOR_MAJOR
					+ count_attacks_major_minor * ATTACKS_MAJOR_MINOR
					+ count_attacks_major_major * ATTACKS_MAJOR_MAJOR
				);
