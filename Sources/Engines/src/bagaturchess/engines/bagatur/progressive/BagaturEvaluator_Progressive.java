...
import bagaturchess.search.api.IEvaluator_Progressive;
import bagaturchess.search.api.internal.ISearch;
import bagaturchess.search.impl.eval.BaseEvaluator;
import bagaturchess.search.impl.eval.BaseEvaluator_cache;
import bagaturchess.search.impl.evalcache.IEvalCache;
import bagaturchess.search.impl.evalcache.IEvalEntry;


public class BagaturEvaluator_Progressive extends BaseEvaluator_Progressive implements IEvaluator_Progressive {
    ...
    private int depth = 0;
    //private double depth_level = 0;
    
    private double[] evals1;
    private double[] evals2;
    private double[] evals3;
    
    
    public BagaturEvaluator_Progressive(IBitBoard _bitboard, IEvalCache _evalCache, IWeightsEvalConfig _evalConfig) {
            super(_bitboard, _evalCache, _evalConfig);
            evalConfig = _evalConfig;
            
            evals1 = new double[ISearch.MAX_DEPTH];
            evals2 = new double[ISearch.MAX_DEPTH];
            evals3 = new double[ISearch.MAX_DEPTH];
    }
    ...
    public void enterLevel(double level) {
    	...
    	if (level <= 0.25) {
    		...
    		evals3[depth] = evals3[depth - 1];
    	}
    	
    	if (level <= 0.5) {
    		evals2[depth] = eval2();
    	} else {
    		evals2[depth] = evals2[depth - 1];
    	}
    	
    	if (level <= 1.0) {
    		evals1[depth] = eval1();
    	} else {
    		evals1[depth] = evals1[depth - 1];
    	}
    ...
    private double eval1() {
		...
		if (factor >= ENDGAME_FACTOR) {
			double eval = eval_material_nopawnsdrawrule();
			return eval;
		} else {
			double eval = eval_material_nopawnsdrawrule();
			return eval;
    ...
    private double eval2() {
    	int factor = bitboard.getMaterialFactor().getTotalFactor();
		if (factor >= ENDGAME_FACTOR) {
			double eval = interpolator.interpolateByFactor(baseEval.getPST_o(), baseEval.getPST_e());
			eval += eval_pawns() + phase3_opening();
			return eval;
		} else {
			double eval = interpolator.interpolateByFactor(baseEval.getPST_o(), baseEval.getPST_e());
			eval += eval_pawns() + phase3_endgame();
			return eval;
		}
	}
	...
	private double evalCache() {
		...
		double eval = evals1[depth] + evals2[depth] + evals3[depth];
		
		
		/*if (USE_CACHE && evalCache != null) {
			evalCache.lock();
			//evalCache.put(hashkey, (int) eval, false);
			evalCache.put(hashkey, 1, (int) eval, 0, 0);
			evalCache.unlock();
		}*/
    ...
    protected double phase3_opening() {
            ...
            //TODO: consider depending on the super class implementation
            //Clear PST scores and calculate them from scratch in eval_pieces in order to apply weights
            eval -= interpolator.interpolateByFactor(baseEval.getPST_o(), baseEval.getPST_e());
            //if (true) throw new IllegalStateException("Who is the super class?");
    ...
    private int eval_pieces() {
            ...
            */
            ...
            BagaturV12PawnsEval pawnsModelEval = (BagaturV12PawnsEval) bitboard.getPawnsStructure();
            /**
            * Pawns iteration
            */
            {
            ...
            */
            ...
            pawnsModelEval = (BagaturV12PawnsEval) bitboard.getPawnsStructure();
