...
public class PSTGenKnights extends PSTGen {
	...
	public static void fillPST(int[] pst, int colour, PawnsModel model, IBitBoard bitboard) {
		...
		int opKingFieldID = colour == Figures.COLOUR_WHITE ? bitboard.getPiecesLists().getPieces(Constants.PID_B_KING).getData()[0] :
																											   bitboard.getPiecesLists().getPieces(Constants.PID_W_KING).getData()[0];
		...
		for (int fromFieldID=0; fromFieldID<64; fromFieldID++) {
			...
			long opAttacks = colour == Figures.COLOUR_WHITE ? model.getBattacks() : model.getWattacks();
			opAttacks |= KingPlies.ALL_KING_MOVES[opKingFieldID];
			
			if ((opAttacks & fromBitboard) != 0L) {
	...
	public static void main(String[] args) {
			...
			int[] pst = new int[64];
			fillPST(pst, Figures.COLOUR_BLACK, ModelBuilder.build(bitboard), bitboard);
