...
public class MoveWrapper {
	...
	public MoveWrapper(int move, boolean isFRC, CastlingConfig castling_cfg) {
		...
		switch (MoveUtil.getMoveType(move)) {
			...
			case MoveUtil.TYPE_CASTLING:
				...
				if (isFRC) { //Chess960/DFRC: emit castling as king-takes-rook; encoded move stays king->target
	...
	public MoveWrapper(String moveString, ChessBoard cb, boolean isFRC) {
		...
		if ("O-O".equals(moveString)) {
			...
			fromIndex 	= cb.color_to_move == ChessConstants.WHITE ? cb.castling_config.from_SquareID_king_w : cb.castling_config.from_SquareID_king_b;
			
			toIndex 	= cb.color_to_move == ChessConstants.WHITE ? CastlingConfig.G1 : CastlingConfig.G8;
			...
			toIndex 	= cb.color_to_move == ChessConstants.WHITE ? CastlingConfig.C1 : CastlingConfig.C8;
		...
		pieceIndex = 
				  (cb.getPieces(cb.color_to_move, ChessConstants.PAWN) & ChessConstants.POWER_LOOKUP[fromIndex]) != 0 ? ChessConstants.PAWN
				: (cb.getPieces(cb.color_to_move, ChessConstants.BISHOP) & ChessConstants.POWER_LOOKUP[fromIndex]) != 0 ? ChessConstants.BISHOP
				: (cb.getPieces(cb.color_to_move, ChessConstants.KNIGHT) & ChessConstants.POWER_LOOKUP[fromIndex]) != 0 ? ChessConstants.KNIGHT
				: (cb.getPieces(cb.color_to_move, ChessConstants.KING) & ChessConstants.POWER_LOOKUP[fromIndex]) != 0 ? ChessConstants.KING
				: (cb.getPieces(cb.color_to_move, ChessConstants.QUEEN) & ChessConstants.POWER_LOOKUP[fromIndex]) != 0 ? ChessConstants.QUEEN
				: (cb.getPieces(cb.color_to_move, ChessConstants.ROOK) & ChessConstants.POWER_LOOKUP[fromIndex]) != 0 ? ChessConstants.ROOK 
				: -1;
		
		if (pieceIndex == -1) {
			
			throw new RuntimeException("Source piece not found at index "
					 + ", cb.pieces[cb.colorToMove][type]=" + cb.getPieces(cb.color_to_move, ChessConstants.KNIGHT)
							 + ", fromIndex=" + fromIndex + ", cb.colorToMove=" + cb.color_to_move + ", move=" + moveString + ", board=" + cb.toString());
		}
		
		
		pieceIndexAttacked = 
				  (cb.getPieces(1 - cb.color_to_move, ChessConstants.PAWN) & ChessConstants.POWER_LOOKUP[toIndex]) != 0 ? ChessConstants.PAWN
				: (cb.getPieces(1 - cb.color_to_move, ChessConstants.BISHOP) & ChessConstants.POWER_LOOKUP[toIndex]) != 0 ? ChessConstants.BISHOP
				: (cb.getPieces(1 - cb.color_to_move, ChessConstants.KNIGHT) & ChessConstants.POWER_LOOKUP[toIndex]) != 0 ? ChessConstants.KNIGHT
				: (cb.getPieces(1 - cb.color_to_move, ChessConstants.KING) & ChessConstants.POWER_LOOKUP[toIndex]) != 0 ? ChessConstants.KING
				: (cb.getPieces(1 - cb.color_to_move, ChessConstants.QUEEN) & ChessConstants.POWER_LOOKUP[toIndex]) != 0 ? ChessConstants.QUEEN
				: (cb.getPieces(1 - cb.color_to_move, ChessConstants.ROOK) & ChessConstants.POWER_LOOKUP[toIndex]) != 0 ? ChessConstants.ROOK 
				: 0;
			...
			if (pieceIndex == ChessConstants.PAWN && (toRank == 1 || toRank == 8)) {
				...
				if (!isCastling && pieceIndex == ChessConstants.KING) {
					...
					if (!isFRC) {
						...
						} else if ((cb.getPieces(cb.color_to_move, ChessConstants.ROOK) & ChessConstants.POWER_LOOKUP[toIndex]) != 0) {
							
							if (toIndex == cb.castling_config.from_SquareID_rook_kingside_w) {
								
								isCastling = true;
								
								toIndex = CastlingConfig.G1;
								
							} else if (toIndex == cb.castling_config.from_SquareID_rook_queenside_w) {
								
								isCastling = true;
								
								toIndex = CastlingConfig.C1;
								
							} else if (toIndex == cb.castling_config.from_SquareID_rook_kingside_b) {
								
								isCastling = true;
								
								toIndex = CastlingConfig.G8;
								
							} else if (toIndex == cb.castling_config.from_SquareID_rook_queenside_b) {
				...
				if (isCastling) {
					...
					if (cb.isValidMove(move)) {
						...
						throw new IllegalStateException("Not valid castling move=" + cb.getMoveOps().moveToString(move) + ", cb=" + cb);
					}
				}
