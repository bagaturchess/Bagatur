...
public final class ChessBoard {
	...
	public void setPinnedAndDiscoPieces() {
		...
		long pinned = 0;
		long discovered = 0;
		final long occupied = allPieces;
		final long[][] boardPieces = pieces;
		...
		for (int kingColor = ChessConstants.WHITE; kingColor <= ChessConstants.BLACK; kingColor++) {
			...
			while (enemyPiece != 0) {
				final long checkerMask = enemyPiece & -enemyPiece;
				final long checkedPiece = ChessConstants.IN_BETWEEN[kingSq][Long.numberOfTrailingZeros(enemyPiece)] & occupied;
				if (checkedPiece != 0 && (checkedPiece & (checkedPiece - 1)) == 0) {
					pinned |= checkedPiece & friendly;
					discovered |= checkedPiece & enemy;
				}
				enemyPiece ^= checkerMask;
			}

		}

		pinnedPieces = pinned;
		discoveredPieces = discovered;
	}
	...
	public boolean isLegal(final int move) {
		...
		if (sourcePieceIndex == KING) {
			return !CheckUtil.isInCheckIncludingKing(toIndex, colorToMove, pieces[colorToMoveInverse],
					allPieces ^ Util.POWER_LOOKUP[fromIndex], MaterialUtil.getMajorPieces(materialKey, colorToMoveInverse));
		}
