...
public class ProbeMain_Stockfish {    
	...
	private static void evaluate(String fen) throws IOException {
		...
		for (int i = 0; i < input.pieces.length; i++) {
			...
			board_array[input.squares[i]] = input.pieces[i];
		}
		
		int eval = NNUEBridge.evalArray(board_array, bitboard.getColourToMove(), bitboard.getDraw50movesRule());
		*/
		
		int eval = NNUEBridge.fasterEvalArray(input.pieces, input.squares,
