	...
	public static void main(String[] args) {
		...
		try {
			...
			while (true) {
				...
				PositionsVisitor trainer = new DeepLearningVisitorImpl_Train_FeedForward(network, output_activation_function);
				
				PositionsTraverser.traverseAll(filePath, trainer, 999999999, input.createBoardConfig(), input.getPawnsEvalFactoryClassName());
				
				saveNetwork(network);
				
				PositionsVisitor success_rate_printer = new DeepLearningVisitorImpl_PrintSuccessRate_FeedForward(output_activation_function);
